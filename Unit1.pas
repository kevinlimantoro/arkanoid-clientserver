...
x:=strtoint(copy(temp,pos('@',temp)+1,3));
y:=strtoint(copy(temp,pos('#',temp)+1,length(temp)-pos('#',temp)));
shape2.Visible:=true;
timer1.Enabled:=true;
clientsocket1.Socket.SendText('now1');
end;
end;
...
clientsocket1.Socket.SendText('kalah');
end
else if (shape2.left+33>form1.width) and (x=10) then
begin
timer1.Enabled:=false;
shape2.Hide;
clientsocket1.Socket.SendText(inttostr(shape2.top)+'!'+inttostr(shape2.left)+'@'+inttostr(x)+'#'+inttostr(y))
end;
if shape2.top+33>=form1.Height then
y:=-10
else if shape2.top<=0 then
y:=10;
if (shape2.Left<=shape1.Left+shape1.Width) and (shape2.top>=shape1.Top) and (shape2.top<=shape1.Top+shape1.Height) then
x:=10;
